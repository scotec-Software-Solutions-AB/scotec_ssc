...
namespace Scotec.Web.ImageServer
{
    public class MagickImageProcessor: IImageProcessor
    {
        public async Task<ImageResponse> ProcessImageAsync(ImageRequest request, IImageProvider imageProvider)
        {
            ...
            if ((request.Width != null && request.Width != image.Width) || (request.Height != null && request.Height != image.Height))
            {
        ...
        private Rectangle CalculateCopyRegion(decimal currentWidth, decimal currentHeight, decimal newWidth, decimal newHeight)
        {
