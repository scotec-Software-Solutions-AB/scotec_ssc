...
internal sealed class MsalTokenCredential : TokenCredential
{
    ...
    /// <summary>
    ///     Synchronously acquires an access cancellationToken for the specified request context.
    /// </summary>
    /// <param name="context">The cancellationToken request context containing scopes and other parameters.</param>
    /// <param name="cancellationToken">A cancellation cancellationToken to cancel the operation.</param>
    ...
    {
        var result = await _authSession.GetTokenSilentAsync(cancellationToken);
