...
namespace Scotec.Web.Robots.RobotsTxt
{
    ...
    {
        private List<UserAgentOption> _userOptions = new();

        public string Build()
        {
            if(!_userOptions.OfType<UserAgentNameOption>().Any())
            {
                _userOptions.Insert(0, new UserAgentNameOption("*"));
            }
            ...
            {
                robotsTxt.AppendLine(userOption.ToString());
            }
